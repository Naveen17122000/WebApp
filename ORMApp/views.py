...
def ormFunction(request):
    if request.method == 'POST':
        ...
        #mp = Employee(empno = eno,empname = ename,salary = esal)
        #emp.save()
        try:
            dept = Deportment.objects.filter(dptno=dno)
            if len(dept)>0:
                Employee.objects.create(empno=eno,empname=ename,salary=esal,deportment=dept[0])
                messages.success(request,'Data inserted successfully')
                #return render(request,'ORMApp/empinsert.html')
                #return HttpResponse('Data saved successfully')
                return redirect('selectempurl')
            else:
                messages.error(request,'department is not available')
                return render(request,'ORMApp/empinsert.html')
        except Exception:
...
def selectEmp(request):
    ...
    emp = Employee.objects.select_related('deportment').all()
    #setofqueries
